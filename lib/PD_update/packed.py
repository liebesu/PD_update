...
def packed(Category,mode):
    ...
    if Category=='md5':
        ...
        if mode=='increment':
            ...
            shutil.copyfile(os.path.join(PD_UPDATE_ROOT,"lib","install","info.json"),os.path.join(tmp_md5_version_path,"info.json"))
            shutil.copyfile(os.path.join(PD_UPDATE_ROOT,"data","md5","result","db_md5_all_{}.md5".format(version)),os.path.join(tmp_md5_version_filepath,"db_md4_all_{}.md5".format(version)))
